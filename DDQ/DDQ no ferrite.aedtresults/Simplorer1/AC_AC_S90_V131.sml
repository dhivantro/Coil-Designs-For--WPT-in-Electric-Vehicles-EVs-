...
INTERN ESINE E_source N1:= net_11 , N2:= net_1 (FREQ:= f, TPERIO:=Tend + 1, AMPL:=311.126983722081, PHASE:=0, PERIO:=0, OFF:=0, TDELAY:=0, SPC:=0, AC_PHASE := 0, AC_MAG := 0.001);
INTERN C Cp N1:= net_5 , N2:= net_2 (C:=Cp);
INTERN C Cs N1:= net_7 , N2:= net_8 (C:=Cs);
INTERN AM I_in N1:= net_2 , N2:= net_1 ( ) ;
INTERN VM V_in N1:= net_5 , N2:= net_11 ( ) ;
INTERN VM V_out N1:= net_7 , N2:= net_10 ( ) ;
...
COUPL gnd gnd2 gnd_term:= net_11 () DST: SIM(Type:=SimVHDL) SRC: DB(File:="Simplorer Elements\VDALibs VHDLAMS\hybrid_emc_vda", Lang:=VHDLA, Lvl:="ideal");
MODEL MaxwellData8_19 Mx_SS1 Rx_in1_src:= net_7 , Rx_in2_src:= net_7 , Rx_in3_src:= net_7 , Tx_in1_src:= net_5 , Tx_in2_src:= net_5 , Tx_in4_src:= net_5 , Rx_in1_snk:= net_10 , Rx_in2_snk:= net_10 , Rx_in3_snk:= net_10 , Tx_in1_snk:= net_11 , Tx_in2_snk:= net_11 , Tx_in4_snk:= net_11 (); 
